...
from point_tracker.pips import Pips
from point_tracker.utils import saverloader


class PipsPointTracker(nn.Module):
    """
    ...
    def _forward(self, rgbs, query_points):
        ...
        start_frames = torch.zeros(n_points, dtype=torch.long, device=rgbs.device)
        visibilities[start_frames, torch.arange(n_points)] = 1.0
        trajectories[start_frames, torch.arange(n_points), :] = query_points[0, :, :]
    ...
    def _forward_once(self, rgbs, query_points):
        """
        Performs forward passes of the PIPS model from left to right
        and returns the predicted trajectories and visibilities.
        """
    ...
    def forward(self, rgbs, query_points):
        ...
        trajectories, visibilities = self._forward(rgbs, query_points)
        return trajectories, visibilities

    def forward_once(self, rgbs, query_points):
        query_points = query_points.float()

        trajectories, visibilities = self._forward_once(rgbs, query_points)
        return trajectories, visibilities
