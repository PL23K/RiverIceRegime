...
import point_tracker.utils.misc
import point_tracker.utils.samp
import point_tracker.utils.improc
...
def balanced_ce_loss(pred, gt, valid=None):
    ...
    pos_loss = point_tracker.utils.basic.reduce_masked_mean(loss, pos * valid)
    neg_loss = point_tracker.utils.basic.reduce_masked_mean(loss, neg * valid)
...
def sequence_loss(flow_preds, flow_gt, vis, valids, gamma=0.8):
    ...
    for i in range(n_predictions):
        ...
        flow_loss += i_weight * point_tracker.utils.basic.reduce_masked_mean(i_loss, valids)
...
class DeltaBlock(nn.Module):
    ...
    def forward(self, fhid, fcorr, flow):
        B, S, D = flow.shape
        assert (D == 3)
        flow_sincos = point_tracker.utils.misc.get_3d_embedding(flow, 64, cat_coords=True)
...
class Pips(nn.Module):
    ...
    def forward(self, xys, rgbs, coords_init=None, feat_init=None, iters=3, trajs_g=None, vis_g=None, valids=None,
        ...
        if feat_init is None:
            ...
            ffeat = point_tracker.utils.samp.bilinear_sample2d(fmaps[:, 0], coords[:, 0, :, 0],
        ...
        if sw is not None and sw.save_this:
            ...
            for s in range(S):
                if trajs_g is not None:
                    ...
                    kp = point_tracker.utils.improc.draw_circles_at_xy(torch.cat([e_, g_], dim=1), H8, W8, sigma=1).squeeze(2)
                    kp = point_tracker.utils.improc.seq2color(kp, colormap='onediff')
                else:
                    kp = point_tracker.utils.improc.draw_circles_at_xy(coords[0:1, s, 0:1], H8, W8, sigma=1).squeeze(2)
                    kp = point_tracker.utils.improc.seq2color(kp, colormap='spring')
                kp = point_tracker.utils.improc.back2color(kp)
                kp_vis.append(kp)
            ...
            if sw is not None and sw.save_this:
                ...
                for s in range(S):
                    ...
                    if trajs_g is not None:
                        ...
                        g_ = trajs_g[0:1, s, 0:1] / float(self.stride)  # 1,1,2, in H8,W8 coords
                        kp = point_tracker.utils.improc.draw_circles_at_xy(torch.cat([e_, g_], dim=1), H8, W8, sigma=1).squeeze(
                            2)
                        kp = point_tracker.utils.improc.seq2color(kp, colormap='onediff')
                    else:
                        kp = point_tracker.utils.improc.draw_circles_at_xy(coords[0:1, s, 0:1], H8, W8, sigma=1).squeeze(2)
        ...
        if sw is not None and sw.save_this:
            ...
            fcps_ = point_tracker.utils.basic.normalize(fcps_)
            for s in range(S):
